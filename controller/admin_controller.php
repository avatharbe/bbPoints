...
class admin_controller
{
	...
	public function display_bbaccounts()
	{
		...
		// ---------------------------------------------------------------
		// Phase C — backfill from existing balances. One-shot, on-demand.
		// Only shown when not yet backfilled AND wallet role is mapped
		// (the minimum for a meaningful opening entry).
		// ---------------------------------------------------------------
		$backfilled = (int) $this->config['ultimatepoints_bbaccounts_backfilled'] === 1;
		$wallet_role_mapped = (int) $this->config['ultimatepoints_acct_user_wallets'] > 0;

		// Equity accounts dropdown for the backfill contra-leg.
		$equity_options = [];
		$bank_role_mapped = (int) $this->config['ultimatepoints_acct_bank_holdings'] > 0;
		$lottery_role_mapped = (int) $this->config['ultimatepoints_acct_lottery_pool'] > 0;

		// POST: run backfill if button submitted and prerequisites met.
		if ($this->request->is_set_post('submit_backfill') && !$backfilled && $wallet_role_mapped)
		{
			if (!check_form_key('acp_points_bbaccounts'))
			{
				trigger_error($this->user->lang['FORM_INVALID'] . adm_back_link($this->u_action), E_USER_WARNING);
			}
			$equity_account_id = $this->request->variable('equity_account_id', 0);
			if ($equity_account_id <= 0)
			{
				trigger_error($this->user->lang['ACP_POINTS_BBACCOUNTS_BACKFILL_NO_EQUITY'] . adm_back_link($this->u_action), E_USER_WARNING);
			}

			$report = $this->run_bbaccounts_backfill($ledger, $equity_account_id, $wallet_role_mapped, $bank_role_mapped, $lottery_role_mapped);
			$this->config->set('ultimatepoints_bbaccounts_backfilled', 1);
			$this->log->add('admin', $this->user->data['user_id'], $this->user->data['user_ip'], 'LOG_BBACCOUNTS_BACKFILL_DONE',
				false,
				[$report['users_processed'], $report['wallet_total'], $report['bank_total'], $report['jackpot_total']]
			);
			trigger_error(sprintf(
				$this->user->lang['ACP_POINTS_BBACCOUNTS_BACKFILL_SUCCESS'],
				$report['users_processed'],
				$report['wallet_total'],
				$report['bank_total'],
				$report['jackpot_total']
			) . adm_back_link($this->u_action));
		}

		if (!$backfilled && $wallet_role_mapped)
		{
			foreach ($ledger->list_accounts('equity') as $row)
			{
				if ((int) $row['is_active'] !== 1)
				{
					continue; // Hide inactive equity accounts from the backfill picker.
				}
				$this->template->assign_block_vars('equity_options', [
					'ACCOUNT_ID'   => (int) $row['account_id'],
					'ACCOUNT_CODE' => $row['account_code'],
					'ACCOUNT_NAME' => $row['account_name'],
					'CURRENCY'     => $row['currency_code'],
		...
		$this->template->assign_vars([
			...
			'S_BBACCOUNTS_BACKFILL_DONE'    => $backfilled,
			'S_BBACCOUNTS_BACKFILL_READY'   => !$backfilled && $wallet_role_mapped && !empty($equity_options),
	...
	/**
	 * Phase C backfill (one-shot, idempotent via the
	 * `ultimatepoints_bbaccounts_backfilled` config flag).
	 *
	 * For each user with a non-zero wallet balance and/or non-zero bank
	 * holding, posts an opening journal entry against the admin-picked
	 * equity account. Posts the *diff* between the legacy denormalised
	 * value and the current bbAccounts subledger balance — handles both
	 * the "B-2 dual-write hasn't started yet" case (post the full amount)
	 * and the "B-2 has been running for a while" case (post the gap).
	 *
	 * Lottery jackpot gets a single entry: Dr equity / Cr lottery_pool.
	 *
	 * @return array{users_processed:int, wallet_total:string, bank_total:string, jackpot_total:string}
	 */
	private function run_bbaccounts_backfill(
		$ledger,
		int $equity_account_id,
		bool $wallet_mapped,
		bool $bank_mapped,
		bool $lottery_mapped
	): array
	{
		...
		$bank_account_id    = (int) $this->config['ultimatepoints_acct_bank_holdings'];
		$lottery_account_id = (int) $this->config['ultimatepoints_acct_lottery_pool'];

		$points_bank_table = $this->phpbb_container->getParameter('dmzx.ultimatepoints.table.points.bank');
		$points_values_table = $this->phpbb_container->getParameter('dmzx.ultimatepoints.table.points.values');

		// One pass over users with non-zero wallet OR non-zero bank.
		// LEFT JOIN so users with only a wallet (no bank row) come through.
		$sql = 'SELECT u.user_id, u.user_points, COALESCE(b.holding, 0) AS bank_holding
		        FROM ' . USERS_TABLE . ' u
		        LEFT JOIN ' . $points_bank_table . " b ON b.user_id = u.user_id
		        WHERE u.user_points <> 0 OR (b.holding IS NOT NULL AND b.holding > 0)
		        ORDER BY u.user_id";
		$result = $this->db->sql_query($sql);
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			if ($wallet_mapped && (float) $row['user_points'] != 0)
			{
				$current = $ledger->get_subledger_balance($wallet_account_id, $user_id);
				$diff = bcsub((string) $row['user_points'], $current, 2);
				if (bccomp($diff, '0', 2) !== 0)
				{
					$lines[] = [
						'account_id'        => $wallet_account_id,
						'debit'             => (bccomp($diff, '0', 2) < 0) ? bcmul($diff, '-1', 2) : '0',
						'credit'            => (bccomp($diff, '0', 2) > 0) ? $diff : '0',
						'subledger_user_id' => $user_id,
					];
					$report['wallet_total'] = bcadd($report['wallet_total'], $diff, 2);
				}
			}

			if ($bank_mapped && (float) $row['bank_holding'] != 0)
			{
				$current = $ledger->get_subledger_balance($bank_account_id, $user_id);
				$diff = bcsub((string) $row['bank_holding'], $current, 2);
				if (bccomp($diff, '0', 2) !== 0)
				{
					$lines[] = [
					...
					$report['bank_total'] = bcadd($report['bank_total'], $diff, 2);
				}
			}
			...
			// Compute the equity contra so the entry balances exactly.
			$dr_total = '0';
			$cr_total = '0';
			foreach ($lines as $l)
			{
				$dr_total = bcadd($dr_total, $l['debit'], 2);
				$cr_total = bcadd($cr_total, $l['credit'], 2);
			}
			$net = bcsub($cr_total, $dr_total, 2);
			$lines[] = [
				'account_id'        => $equity_account_id,
				'debit'             => (bccomp($net, '0', 2) > 0) ? $net : '0',
				'credit'            => (bccomp($net, '0', 2) < 0) ? bcmul($net, '-1', 2) : '0',
				'subledger_user_id' => 0,
			];
			...
			try
			{
				$ledger->create_entry(
					$now,
					'Phase C opening balance for user ' . $user_id,
					$lines,
					'manual',
					$user_id,
					'dmzx.ultimatepoints',
			...
			{
				$this->log->add('critical', (int) $this->user->data['user_id'], $this->user->data['user_ip'],
					'LOG_BBACCOUNTS_POST_FAILED',
					false,
					['backfill/user_' . $user_id, $e->getMessage()]
				);
			}
		}
		$this->db->sql_freeresult($result);

		// Lottery jackpot — single entry, no subledger.
		if ($lottery_mapped)
		{
			// `points_values` is single-row wide-format — each setting is its own column.
			$sql = 'SELECT lottery_jackpot FROM ' . $points_values_table;
			$row = $this->db->sql_fetchrow($this->db->sql_query_limit($sql, 1));
			$jackpot = $row ? (string) $row['lottery_jackpot'] : '0';
			$current = $ledger->get_account_balance($lottery_account_id);
			$diff = bcsub($jackpot, $current, 2);
			if (bccomp($diff, '0', 2) !== 0)
			{
				try
				{
					$ledger->create_entry(
						$now,
						'Phase C opening balance for lottery jackpot',
						[
							[
								'account_id' => $lottery_account_id,
								'debit'      => (bccomp($diff, '0', 2) < 0) ? bcmul($diff, '-1', 2) : '0',
								'credit'     => (bccomp($diff, '0', 2) > 0) ? $diff : '0',
							],
							[
								'account_id' => $equity_account_id,
								'debit'      => (bccomp($diff, '0', 2) > 0) ? $diff : '0',
								'credit'     => (bccomp($diff, '0', 2) < 0) ? bcmul($diff, '-1', 2) : '0',
							],
						],
						'manual',
						0,
